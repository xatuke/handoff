...
public:
        ...
        : QObject(parent), airpodsMac(airpodsMac)
    {
        ...
        std::cout << "[Handoff] Local MAC (reversed): " << localMac.toHex().toStdString() << std::endl;
...
private slots:
    void onConnected() {
        std::cout << "[Handoff] Connected to AirPods" << std::endl;
    ...
    void onDataReceived() {
        ...
        if (data.startsWith(Packets::Connection::FEATURES_ACK)) {
            std::cout << "[Handoff] Received FEATURES_ACK - requesting notifications" << std::endl;
        ...
        if (data.startsWith(Packets::AudioSource::HEADER)) {
            ...
            if (newSource.isValid) {
                ...
                std::cout << "[Handoff] Audio source: " << newSource.deviceMac.toHex().toStdString()
                ...
                if (newSource.type == Packets::AudioSource::NONE) {
                    if (shouldReclaimOnNone) {
                        std::cout << "[Handoff] Another device released audio - reclaiming" << std::endl;
                ...
                // Another device has audio
                    ...
                    if (weHadAudio) {
                        std::cout << "[Handoff] Another device took audio from us - pausing Linux" << std::endl;
                        media->pauseAllMedia();
                    ...
                    // If Linux has any active audio (MPRIS or Discord/games), mark for reclaim
                        ...
                        std::cout << "[Handoff] Another device has audio and Linux has active audio - marking for reclaim" << std::endl;
    ...
    void onPlaybackStarted() {
        ...
        if (currentSource.isValid) {
            if (currentSource.type == Packets::AudioSource::NONE) {
                std::cout << "[Handoff] Playback started - no device has audio" << std::endl;
                return;
            }

            std::cout << "[Handoff] Comparing MACs - Current: " << currentSource.deviceMac.toHex().toStdString()
                     << ", Local: " << localMac.toHex().toStdString() << std::endl;

            if (currentSource.deviceMac == localMac) {
                std::cout << "[Handoff] We already own audio, no handoff needed" << std::endl;
                return;
            }

            std::cout << "[Handoff] Another device has audio - reclaiming" << std::endl;
        } else {
            std::cout << "[Handoff] Playback started - no AUDIO_SOURCE info yet, skipping" << std::endl;
            return;
        }

        // Claim ownership and reclaim audio stream
        if (socket && socket->isOpen()) {
            std::cout << "[Handoff] Sending OWNS_CONNECTION (claim)" << std::endl;
            socket->write(Packets::OwnsConnection::CLAIM);
    ...
    void onError(QBluetoothSocket::SocketError error) {
        std::cerr << "[Handoff] Socket error: " << static_cast<int>(error) << std::endl;
    }

private:
    void connectToAirPods() {
        std::cout << "[Handoff] Connecting to AirPods..." << std::endl;
        ...
        connect(socket, &QBluetoothSocket::disconnected, this, [this]() {
            std::cerr << "[Handoff] Socket disconnected!" << std::endl;
        });
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "[Main] AirPods MAC: " << airpodsMac.toStdString() << std::endl;
